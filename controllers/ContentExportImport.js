...
module.exports = {
	importContent: async (ctx) => {
		...
		const validationResult = validator.validateImportContentRequest(ctx.request.body)
		if (validationResult) {
	...
	deleteAllContent: async (ctx) => {
		...
		const count = await importService.deleteAllData(ctx.request.body.targetModelUid, ctx)
		ctx.send({
			message: 'ok',
			count
		})
