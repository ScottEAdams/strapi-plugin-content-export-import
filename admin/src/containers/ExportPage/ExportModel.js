...
const ExportModel = ({ model }) => {
	...
	const downloadJson = () => {
		...
		const file = new File([JSON.stringify(content)], `${model.apiID}-${current.getTime()}.json`, {
	...
	return (
		<ModelItem>
			<HFlex>
				...
				<div>
					<Button disabled={fetching} loader={fetching} onClick={fetchModelData} secondaryHotline>
